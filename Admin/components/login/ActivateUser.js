...
const ActivateUser = ({ activate, password, email, cancel, userId }) => {
    const [code, setCode] = useState('');

    const { mutateAsync, isLoading } = useMutation(AuthService.activateUser, {
        onSuccess: async (data) => {
            if (data?.errors?.status === 400) {
                errorNotify('Incorrect code');
                return;
            }
            successNotify('User has been activated');
    ...
    const { mutateAsync: resendCode } = useMutation(AuthService.resendCode, {
        onSuccess: async (data) => {
            if (data?.errors?.status === 400) {
